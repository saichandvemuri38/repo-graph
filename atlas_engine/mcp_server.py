...
def build_server(root: Path):
    ...
    @tool()
    def atlas_search(text: str, limit: int = 15) -> str:
        """Find symbols by name or concept (full-text, camelCase and snake_case aware). Start here when you only have a keyword."""
        return reports.md_search(text, fresh().search(text, limit))
