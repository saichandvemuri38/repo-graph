...
from .indexer import index, scan_changes, sha12
...
class Atlas:
    ...
    def impact(self, target: str, direction: str = "upstream", depth: int = 3) -> dict:
        ...
        return {
            "target": target, "kind": kind, "direction": direction, "seeds": seed_ids, "levels": levels, "truncated": truncated,
    ...
    def changes(self, base: str | None = None) -> dict:
        ...
        if git:
            ...
            for path, info in list(files.items()):
                ...
                if EXT_LANG.get(ext) not in usable:
                    del files[path]
                    continue
                lang = EXT_LANG[ext]
