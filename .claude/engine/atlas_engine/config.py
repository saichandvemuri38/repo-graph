...
APP_DIR = ".claude/atlas"          # generated data (graph database, work ledger, HTML report) lives inside .claude
SCHEMA_VERSION = "1"
...
EXCLUDE_DIRS = {
    ...
    "vendor", ".next", ".idea", ".vscode", "site-packages", ".claude",
}
...
class Paths:
    root: Path
    data: Path | None = None   # where graph data and generated pages live; default is <root>/.claude/atlas
...
def find_root(start: str | os.PathLike | None = None) -> Path:
    """Project root: $CODEATLAS_ROOT, else the nearest ancestor holding a .claude/atlas folder, else `start`."""
