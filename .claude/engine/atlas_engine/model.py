...
@dataclass
class DynSite:
    """Something the graph cannot follow: a call that goes through a name looked up at run time, or a
    function handed to a framework by a decorator. Stored so "nothing calls this" can be checked honestly."""
    src: str           # symbol id the site sits in (or is attached to)
    kind: str          # decorator | getattr-literal | getattr-dynamic | dynamic-import | eval | namespace-lookup
    detail: str
    line: int
...
class FileParse:
    ...
    error: str = ""
    dyn: list[DynSite] = field(default_factory=list)
    strings: list[tuple[str, int, str]] = field(default_factory=list)   # (identifier-like string, line, symbol id)
