...
-> clusters + processes -> export text shards.
...
def is_indexable_path(rel: str, patterns: list[str]) -> bool:
    """False for folders the indexer never reads (.claude, node_modules, ...) and for `.codeatlasignore` matches."""
    return not (any(p in EXCLUDE_DIRS for p in rel.split("/")[:-1]) or _ignored(rel, patterns))
...
def discover(root: Path) -> list[str]:
    ...
    for rel in sorted(set(candidates)):
        parts = rel.split("/")
        if not is_indexable_path(rel, patterns):
            continue
        ...
        try:
            ...
            with store.tx():
                ...
                if structure_changed:
                    ...
                    store.set_meta(lastLink=now, lastCluster=now, clusterAlgo=result.cluster_algo, linkCount=int(meta.get("linkCount", 0) or 0) + 1)
