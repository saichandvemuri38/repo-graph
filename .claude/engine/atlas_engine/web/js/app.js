// CodeAtlas Explorer: wires the graph canvas, search, panels and modals together.
...
import { helpModal, settingsModal, changesModal, checksModal, flowModal, parseErrorsModal, skippedModal, closeTopModal, hasModal } from "./modals.js";
...
const S = {
  ...
  expanded: new Set(), selected: null, hl: null, hlOn: true, processes: [], clusters: [], codeCache: new Map(),
  loadToken: 0, layout: "idle", showTests: false, polling: null, indexing: false, lastQuery: "", codeView: { file: null },
  prefs: { theme: "auto", colorBy: "kind", labelDensity: 0.9, rightWidth: 400 },
};
...
async function openProject(name) {
  ...
  try {
    ...
    if (!S.status.counts.files) {
      showOverlay(h("div", { class: "box" }, h("h3", {}, "Nothing indexed yet"), h("p", { class: "muted" }, "Run: pwsh -NoProfile -File .claude/scripts/atlas.ps1 report")));
      renderAll(); return;
    }
    ...
    if (estimate > LARGE_GRAPH) {
      showOverlay(h("div", { class: "box" }, h("h3", {}, "Large project"), h("p", { class: "muted" }, `This project has about ${estimate.toLocaleString()} nodes. Drawing all of them can make the browser slow.`),
        ...
        h("p", { class: "muted small" }, "Search, details and flows work without the full graph.")));
      renderAll();
      loadSideData(token);
      return;
    }
...
function renderProjectMenu() {
  ...
  const p = S.projects.find((x) => x.name === S.project) || S.projects[0];
  const st = S.status;
  fill(menu,
    h("div", { class: "dd-item sel" }, h("div", { class: "grow" },
      h("div", { class: "t" }, p ? p.name : "", h("span", { class: "badge ok", style: { marginLeft: "6px" } }, "report")),
      h("div", { class: "s" }, p && p.stats ? `${p.stats.files} files · ${p.stats.symbols} symbols` : ""),
      h("div", { class: "s" }, p ? p.root : ""))),
    h("div", { class: "dd-sep" }),
    h("div", { class: "dd-empty", style: { textAlign: "left" } }, `Snapshot made ${st && st.generated ? fmtAge(st.generated) : ""}. This page is a saved copy of the graph; refresh it with: pwsh -NoProfile -File .claude/scripts/atlas.ps1 report`));
}
...
function landing() {
  S.project = null;
  $("#project-name").textContent = "No graph";
  showOverlay(h("div", { class: "box" }, h("h3", {}, "This report has no graph"), h("p", { class: "muted" }, "Run: pwsh -NoProfile -File .claude/scripts/atlas.ps1 report")));
}
...
function renderStatus() {
  ...
  fill(el, 
    h("span", { class: "ok", title: "A saved copy of the graph. Regenerate it to see new code." }, "● Report"),
    ...
    h("span", { title: st.generated || st.meta.lastLink }, `Snapshot ${fmtAge(st.generated || st.meta.lastLink)}`),
...
async function renderDetails(id) {
  ...
  if (!id) {
    fill(host, h("div", { class: "empty" }, "Select a node to see its details.", h("div", { class: "small", style: { marginTop: "6px" } }, "Click a node in the graph, the explorer, or search results.")),
      h("h4", {}, "Try"), h("div", { class: "muted small" }, "• Double-click a node to focus its neighbourhood", h("br"), "• Ctrl/⌘ K to search"));
    return null;
  }
...
function symbolDetails(d) {
  ...
  if (d.dynamic && d.dynamic.evidence.length) out.push(section("Possible use the graph cannot see", d.dynamic.evidence.map((e) => h("div", { class: "list-item", title: e.kind }, h("span", { class: "badge " + (e.strength === "strong" ? "ok" : "warn") }, e.strength === "strong" ? "likely" : "lead"), h("span", { class: "grow small" }, e.text)))));
...
let searchSel = -1, searchHits = [], searchSeq = 0, related = false;
const relatedBtn = $("#btn-related");
relatedBtn.addEventListener("click", () => {
  related = !related;
  relatedBtn.setAttribute("aria-pressed", String(related));
  search.placeholder = related ? "Describe what you are looking for…" : "Search nodes…  (Ctrl/⌘ K)";
  search.focus();
  doSearch();
});
...
async function doSearch() {
  ...
  try {
    const r = await api(`/p/${P()}/search?q=${encodeURIComponent(q)}&limit=25${related ? "&mode=graph" : ""}`);
...
function drawSearch() {
  ...
  fill(results, ...searchHits.map((r, i) => h("div", { class: "dd-item" + (i === searchSel ? " sel" : ""), role: "option", on: { mousedown: (e) => { e.preventDefault(); pickSearch(i); }, mouseenter: () => { searchSel = i; $$(".dd-item", results).forEach((el, j) => el.classList.toggle("sel", j === i)); } } },
    h("span", { class: "dot " + r.kind }), h("div", { class: "grow" }, h("div", { class: "t" }, r.label), h("div", { class: "s" }, r.kind === "folder" ? r.file : `${r.file}${r.line ? ":" + r.line : ""}`), r.why ? h("div", { class: "why" }, r.why) : null), h("span", { class: "badge" }, r.kind))),
...
$("#btn-checks").addEventListener("click", () => S.project && checksModal(ctx()));
...
function restartPolling() { /* a saved report does not change; regenerate it to see new code */ }
...
(async function boot() {
  ...
  try {
    ...
    showOverlay(h("div", { class: "box" }, h("h3", {}, "Could not read this report"), h("p", { class: "muted" }, e.message), h("button", { class: "btn", on: { click: () => location.reload() } }, "Reload")));
  }
})();
