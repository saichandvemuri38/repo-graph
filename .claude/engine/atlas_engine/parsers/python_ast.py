...
from ..model import DynSite, FileParse, ImportBinding, RawEdge, Symbol

MAX_SIGNATURE = 160
MAX_SUMMARY = 100

# Decorators that only change how a function behaves; they do not hand it to a framework.
BENIGN_DECORATORS = {
    "property", "staticmethod", "classmethod", "abstractmethod", "abc.abstractmethod", "overload", "typing.overload",
    "final", "typing.final", "override", "typing.override", "dataclass", "dataclasses.dataclass", "wraps", "functools.wraps",
    "cache", "functools.cache", "lru_cache", "functools.lru_cache", "cached_property", "functools.cached_property",
    "contextmanager", "contextlib.contextmanager", "asynccontextmanager", "contextlib.asynccontextmanager",
    "total_ordering", "functools.total_ordering", "singledispatch", "functools.singledispatch",
}
_IDENT_STRING = re.compile(r"[A-Za-z_][A-Za-z0-9_.]{0,79}")
MAX_STRINGS = 400
...
def _looks_like_class(dotted: str) -> bool:
    return dotted.rsplit(".", 1)[-1].lstrip("_")[:1].isupper()      # `_Private` is a class too
...
class _Extractor(ast.NodeVisitor):
    ...
    def _decorator(self, sym_id: str, dec: ast.expr) -> None:
        ...
        if target:
            ...
            if last in ("property", "cached_property", "setter", "getter", "deleter"):
                self.fp.dyn.append(DynSite(sym_id, "property", target, dec.lineno))
            elif target not in BENIGN_DECORATORS and last not in ("setter", "getter", "deleter", "register"):
                self.fp.dyn.append(DynSite(sym_id, "decorator", target, dec.lineno))
            elif last == "register":                       # singledispatch-style registration hands the function over too
                self.fp.dyn.append(DynSite(sym_id, "decorator", target, dec.lineno))
    ...
    def _dynamic_call(self, node: ast.Call) -> None:
        name = _dotted(node.func) or ""
        last = name.rsplit(".", 1)[-1]
        src = self.scope.id
        if name in ("getattr", "setattr", "hasattr", "delattr") and len(node.args) >= 2:
            key = node.args[1]
            prefix = _name_prefix(key)
            if isinstance(key, ast.Constant) and isinstance(key.value, str):
                self.fp.dyn.append(DynSite(src, "getattr-literal", key.value, node.lineno))
            elif prefix:
                self.fp.dyn.append(DynSite(src, "getattr-prefix", prefix, node.lineno))
                self.fp.dyn.append(DynSite(src, "getattr-dynamic", _unparse(key), node.lineno))
            else:
                self.fp.dyn.append(DynSite(src, "getattr-dynamic", _unparse(key), node.lineno))
        elif last in ("import_module", "__import__") or name in ("importlib.import_module", "__import__"):
            arg = node.args[0] if node.args else None
            detail = arg.value if isinstance(arg, ast.Constant) and isinstance(arg.value, str) else _unparse(arg)
            self.fp.dyn.append(DynSite(src, "dynamic-import", detail, node.lineno))
        elif name in ("eval", "exec"):
            self.fp.dyn.append(DynSite(src, "eval", name, node.lineno))

    def visit_Subscript(self, node: ast.Subscript) -> None:
        base = node.value
        if isinstance(base, ast.Call) and _dotted(base.func) in ("globals", "locals", "vars"):
            self.fp.dyn.append(DynSite(self.scope.id, "namespace-lookup", f"{_dotted(base.func)}()[{_unparse(node.slice)}]", node.lineno))
        elif _dotted(base) == "sys.modules":
            self.fp.dyn.append(DynSite(self.scope.id, "namespace-lookup", f"sys.modules[{_unparse(node.slice)}]", node.lineno))
        self.generic_visit(node)

    def visit_Expr(self, node: ast.Expr) -> None:
        if isinstance(node.value, ast.Constant) and isinstance(node.value.value, str):
            return                                          # docstrings and bare strings are prose, not names
        self.generic_visit(node)
    ...
    def visit_Constant(self, node: ast.Constant) -> None:
        v = node.value
        if isinstance(v, str) and len(self.fp.strings) < MAX_STRINGS and _IDENT_STRING.fullmatch(v):
            self.fp.strings.append((v, node.lineno, self.scope.id))

    def visit_Call(self, node: ast.Call) -> None:
        self._dynamic_call(node)
...
def _name_prefix(node: ast.AST) -> str:
    """The fixed start of a computed name: 'on_' + kind, f"visit_{kind}", "do_%s" % kind. Empty when there is none."""
    if isinstance(node, ast.BinOp) and isinstance(node.op, (ast.Add, ast.Mod)):
        left = node.left
        if isinstance(left, ast.Constant) and isinstance(left.value, str):
            return left.value.split("%", 1)[0]
