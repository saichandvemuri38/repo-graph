...
CREATE TABLE IF NOT EXISTS dynamic(file TEXT, src TEXT, kind TEXT, detail TEXT, line INTEGER);
CREATE INDEX IF NOT EXISTS dynamic_file ON dynamic(file);
CREATE INDEX IF NOT EXISTS dynamic_src ON dynamic(src);
CREATE TABLE IF NOT EXISTS strings(file TEXT, src TEXT, value TEXT, line INTEGER);
CREATE INDEX IF NOT EXISTS strings_file ON strings(file);
CREATE INDEX IF NOT EXISTS strings_value ON strings(value);
...
class Store:
    ...
    def remove_file(self, path: str) -> None:
        ...
        for table in ("symbols", "raw_edges", "imports", "edges", "unresolved", "dynamic", "strings"):
    ...
    def replace_file(self, fp: FileParse, size: int, mtime_ns: int, indexed_at: str) -> None:
        ...
        self.db.executemany(
            "INSERT INTO dynamic(file, src, kind, detail, line) VALUES (?,?,?,?,?)",
            [(fp.path, d.src, d.kind, d.detail, d.line) for d in fp.dyn],
        )
        self.db.executemany(
            "INSERT INTO strings(file, src, value, line) VALUES (?,?,?,?)",
            [(fp.path, src, value, line) for value, line, src in fp.strings],
        )
        self.db.executemany(
