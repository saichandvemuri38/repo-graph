---
name: atlas-impact
description: Blast-radius analysis before changing code. Use when the user asks "what breaks if I change X", "is it safe to edit X", or before editing any function, class or method. Saves an HTML report.
...
1. Run `sh scripts/codeatlas.sh impact $ARGUMENTS --save` (or the MCP tool `atlas_impact`; then save with `sh scripts/codeatlas.sh impact <target> --save`). The engine refreshes the graph, walks dependants to depth 3, scores risk with the rubric in `.claude/prompts/risk-rubric.md`, and writes an HTML report.
...
5. Reply with: the risk level, the top direct dependants, flows touched, your suggested checks, and the path of the saved HTML report (printed by the command).
