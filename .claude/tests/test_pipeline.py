...
def test_broken_file_is_recovered_not_dropped(project):
    result = index(project, full=True)
...
def test_changes_with_git_compares_against_head(project):
    ...
    git("init", "-q")
    (project / ".gitignore").write_text(".claude/atlas/\n")
    git("add", "-A")
    git("commit", "-qm", "base")
    index(project, full=True)
    write(project, {"pkg/util.py": (
...
def test_concurrent_index_runs_are_serialised(project):
    import threading
    results = []
    threads = [threading.Thread(target=lambda: results.append(index(project, full=True))) for _ in range(3)]
...
def test_indexing_writes_the_graph_and_no_html(tmp_path):
    (tmp_path / ".claude" / "atlas").mkdir(parents=True)
    write(tmp_path, {"a.py": "def f():\n    return 1\n"})
    index(tmp_path, full=True)
    assert not list(Paths(tmp_path).app.rglob("*.html"))
    assert (Paths(tmp_path).graph / "atlas.db").is_file()
    assert Paths(tmp_path).graph == tmp_path / ".claude" / "atlas" / "graph"


def test_changes_ignore_folders_the_index_never_reads(project):
    def git(*a):
        subprocess.run(["git", "-C", str(project), "-c", "user.email=t@t", "-c", "user.name=t", *a], check=True, capture_output=True)
    git("init", "-q")
    git("add", "-A")
